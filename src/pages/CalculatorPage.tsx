        ...
        <div className="mb-4 relative">
          ...
          <div className="relative">
            <select 
              ...
              className={`w-full appearance-none ${lightBg} rounded-xl p-3 pr-10 text-base font-bold text-slate-800 outline-none transition-all`}
            >
              {banks.map(b => (
        ...
        <div className="mb-5">
          ...
          <div className="relative">
            <input 
              ...
              placeholder="0"
              className={`w-full ${lightBg} rounded-xl p-3 pr-10 text-lg font-black text-slate-900 outline-none transition-all placeholder:text-slate-300`}
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-500 pointer-events-none">원</span>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="w-full max-w-[480px] min-h-screen flex flex-col relative bg-[#F8FAFF] sm:shadow-[0_0_80px_rgba(0,0,0,0.03)]">
        ...
        {/* Header */}
        <header className="w-full h-14 px-4 flex items-center justify-between sticky top-0 bg-[#F8FAFF]/90 backdrop-blur-md z-10 border-b border-slate-100/50">
          <button onClick={onBack} className="p-1.5 text-slate-900">
            <ChevronLeft size={24} strokeWidth={2.5} />
          </button>
          <h1 className="text-[15px] font-bold text-slate-900">적금 비교 계산기</h1>
          <button onClick={onReset} className="text-[13px] font-bold text-blue-600 px-1.5">
            초기화
          </button>
        </header>
